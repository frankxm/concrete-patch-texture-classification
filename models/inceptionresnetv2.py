...
class InceptionResNetV2(nn.Module):
    ...
    def __init__(self, num_classes=1001,use_amp=False):
        ...
        # 添加bn层,dropout，最后阶段的卷积层，特征表示非常深，很容易过拟合
        self.conv2d_7b = nn.Sequential(
    ...
    def forward(self, input,step='train'):
...
def inceptionresnetv2(num_classes=1000, pretrained='imagenet'):
    ...
    if pretrained:
        ...
        w=model_zoo.load_url(settings['url'])
...
def measure_latency_cpu(model, batch_size=64):
    ...
    dummy_input = torch.randn(batch_size, 3, 299, 299, dtype=torch.float).to(device)
    model.to(device)
...
if __name__ == '__main__':
    ...
    # 54.31M
    net1 = InceptionResNetV2(num_classes=4)
    net2= InceptionResNetV2(num_classes=4)
    net3 = InceptionResNetV2(num_classes=4)
    net4 = InceptionResNetV2(num_classes=4)
    net5 = InceptionResNetV2(num_classes=4)
