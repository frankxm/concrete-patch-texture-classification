...
def plot_confusion_matrix(cm_list, type_list, ground_classes_names, evaluation_path):
    ...
    for index, cm in enumerate(cm_list):
        ...
        disp = ConfusionMatrixDisplay(
            confusion_matrix=cm,
            display_labels=ground_classes_names
        )
        ...
        save_path = os.path.join(
            evaluation_path,
            f"confusion_matrix_{cm_type}.png"
        )
...
# Macro-f1适合类别不平衡，每个类别都考虑。关注小样本准确率用Macro 数据集 严重不平衡 少数类性能非常重要（医学检测的病类、缺陷检测中的异常类）
# Micro-f1偏向多数类，适合类别平衡。 在多标签任务中，每个样本有多个标签，用 micro 比较合理  单标签任务中：想看整体性能，而不是关注小类
# weighted-f1 回归到 真实数据分布 多数类影响大 少数类被弱化 在不平衡数据中按真实样本分布看整体表现（也是多数类主导）
